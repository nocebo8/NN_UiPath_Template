#!/usr/bin/env bash
set -euo pipefail

REPO_PATH="${1:-/Users/michalmichalak/Documents/NN_UiPath_Template}"
BRANCH="${2:-ci/test-pack-$(date +%s)}"
PR_TITLE="${3:-CI: UiPath pack check}"
PR_BODY="${4:-Triggering UiPath pack check to validate project packing.}"
WORKFLOW_DISPLAY_NAME="UiPath Compile Check"

if [ -z "${GITHUB_TOKEN:-}" ]; then
  echo "ERROR: GITHUB_TOKEN must be set in environment"; exit 1
fi

cd "$REPO_PATH"
REMOTE_URL=$(git remote get-url origin)
if [[ "$REMOTE_URL" =~ ^git@github.com:(.*)/(.*)\.git$ ]]; then
  OWNER=${BASH_REMATCH[1]}; REPO=${BASH_REMATCH[2]}
elif [[ "$REMOTE_URL" =~ ^https://github.com/(.*)/(.*)(.git)?$ ]]; then
  OWNER=${BASH_REMATCH[1]}; REPO=${BASH_REMATCH[2]}
else
  echo "Cannot parse origin remote url: $REMOTE_URL"; exit 1
fi
API="https://api.github.com/repos/$OWNER/$REPO"

git fetch origin master
git checkout -b "$BRANCH" origin/master
git commit --allow-empty -m "CI: trigger UiPath pack check $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
git push -u origin "$BRANCH"

PR_RESPONSE=$(curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" \
  -X POST "$API/pulls" \
  -d "{\"title\":\"$PR_TITLE\",\"head\":\"$BRANCH\",\"base\":\"master\",\"body\":\"$PR_BODY\"}" || true)

PR_URL=$(echo "$PR_RESPONSE" | jq -r .html_url 2>/dev/null || true)
if [ -z "$PR_URL" ] || [ "$PR_URL" = "null" ]; then
  PR_INFO=$(curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" \
    "$API/pulls?state=open&head=$OWNER:$BRANCH")
  PR_URL=$(echo "$PR_INFO" | jq -r '.[0].html_url')
fi
echo "PR: $PR_URL"

# wait for run
sleep 5
MAX_WAIT=900; INTERVAL=8; ELAPSED=0; RUN_ID=""
while [ $ELAPSED -lt $MAX_WAIT ]; do
  RUNS_JSON=$(curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" \
    "$API/actions/runs?per_page=50&branch=$BRANCH")
  RUN_ID=$(echo "$RUNS_JSON" | jq -r --arg wf "$WORKFLOW_DISPLAY_NAME" '.workflow_runs[] | select(.name==$wf) | .database_id' | head -n1 || true)
  if [ -n "$RUN_ID" ] && [ "$RUN_ID" != "null" ]; then break; fi
  sleep $INTERVAL
  ELAPSED=$((ELAPSED + INTERVAL))
done

if [ -z "$RUN_ID" ]; then
  echo "No workflow run found"; exit 1
fi

echo "Run id: $RUN_ID - waiting for completion..."
ELAPSED=0
while [ $ELAPSED -lt $MAX_WAIT ]; do
  RUN_INFO=$(curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" "$API/actions/runs/$RUN_ID")
  STATUS=$(echo "$RUN_INFO" | jq -r .status)
  CONCL=$(echo "$RUN_INFO" | jq -r .conclusion)
  echo "Status: $STATUS, Conclusion: $CONCL"
  [ "$STATUS" = "completed" ] && break
  sleep $INTERVAL
  ELAPSED=$((ELAPSED + INTERVAL))
done

OUTDIR="$(pwd)/ci-run-results-$RUN_ID"
mkdir -p "$OUTDIR"
curl -L -H "Authorization: token $GITHUB_TOKEN" -o "$OUTDIR/run_logs.zip" "$API/actions/runs/$RUN_ID/logs"
unzip -q "$OUTDIR/run_logs.zip" -d "$OUTDIR/logs" || true
ARTS=$(curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github+json" "$API/actions/runs/$RUN_ID/artifacts")
echo "$ARTS" | jq -r '.artifacts[] | [.id,.name] | @tsv' | while IFS=$'\t' read -r id name; do
  curl -L -H "Authorization: token $GITHUB_TOKEN" -o "$OUTDIR/artifacts/${name}_${id}.zip" "$API/actions/artifacts/$id/zip"
done

echo "Results: $OUTDIR"
